import { OPENAI_API_KEY, PUBLIC_POCKETBASE_INSTANCE } from '$env/static/private';
...
async function getFileContent(fileRecord: typeof contentLibrary.$inferSelect, customFetch: typeof fetch): Promise<string> {
	...
	if (fileRecord.file.startsWith('http')) {
		...
		fileUrl = `${PUBLIC_POCKETBASE_INSTANCE}/api/files/content_library/${fileRecord.id}/${fileRecord.file}`;
	}
		...
		if (assistant) {
			...
			if (assistant.trainingFiles && assistant.trainingFiles.length > 0) {
				...
				for (const fileRecord of fileRecords) {
					...
					const fileContent = await getFileContent(fileRecord, fetch);
					if (fileContent) {
